...
from helpers.back_test.walk_forward.bt_wf import run_walk_forward  # Walk-forward backtesting helper
...
max_hold_candles_list = list(range(5, 36, 10))                              # Max hold durations
vwap_windows = list(range(5, 36, 10))                                       # VWAP rolling window sizes
fixed_sl_values = [0.01, 0.02, 0.03]                                        # Stop-loss values
cooldown_candle_options = list(range(3, 16, 6))                              # Cooldown periods

# === Random parameter combinations generator ===
def generate_random_param_combinations():
    """
    Generates a list of dictionaries with random strategy parameters.
    """
    return [dict(
...
def vwap_strategy(df, vwap_distance_threshold, vwap_exit_threshold, max_hold_candles,
    ...
    """
    Simulates trades using a VWAP-based strategy with stop-loss and cooldown logic.
    Returns percent gains per trade, max drawdown, average holding time, and number of trades.
    """
    cash, asset = 1000.0, 0.0                  # Initial capital and asset holding
    hold_candles = 0                            # Number of candles current trade has been held
    cooldown_counter = 0                        # Cooldown period counter after stop-loss
    entry_price = 0                             # Entry price for current trade
    ...
    for i in range(1, len(df)):
        price = open_[i]        # Current candle open price
        ...
        if asset == 0 and price < vw * (1 - vwap_distance_threshold):
            entry_price = price
            asset = (cash * (1 - fee_rate)) / entry_price  # Buy as much as possible
            cash = 0
            hold_candles = 0
        ...
        # --- EXIT CONDITIONS ---
            ...
            # Determine exit price based on stop-loss, VWAP exit, or max hold time
            if low[i] <= stop_loss_price:
                exit_price = stop_loss_price
...
# === Helper: round floats in nested structures ===
...
# === Wrapper for walk-forward backtest ===
def strategy_wrapper(df, fee_rate, **kwargs):
    """
    Wrapper that passes fee_rate to the VWAP strategy for walk-forward execution.
    """
...
def worker_run_walk_forward(combo, base_folder, fee_rate, param_combinations):
    """
    Calls walk-forward backtesting on a single symbol/interval combination.
    """
...
def main():
    # Load symbol list and data paths
    csv_path = os.path.join(base_dir, PROJECT_ROOT, "symbols.csv")
    base_folder = os.path.abspath(os.path.join(base_dir, PROJECT_ROOT, "data"))
    ...
    # Sort results by average gain per trade
