...
log_file = "live_trading_robust_long.log"
...
# === Binance client ===
api_key = os.getenv("BINANCE_API_KEY_SUB2")
api_secret = os.getenv("BINANCE_API_SECRET_SUB2")
...
# === Load symbols ===
param_path = os.path.join(PROJECT_ROOT, "momentum_strategy", "backtest", "longParamsRobust.json")
        ...
        if not getattr(bot, "in_position", False):
            if mom > 0 and vol_today > bot.volume_factor * vol_avg and is_green:
                ...
                await submit_order(bot.client.futures_create_order,
                                   ...
                                   recvWindow=5000)
                logging.info(f"[{time_str}] {bot.upper_symbol} LONG BUY at ~{price} | Open positions: {OPEN_POSITIONS_COUNT}")
            ...
            if mom < 0 or held >= bot.max_hold_candles:
                ...
                logging.info(f"[{time_str}] {bot.upper_symbol} SELL at ~{price} | Held {held} candles | Open positions: {OPEN_POSITIONS_COUNT}")
                bot.entry_time = None
                bot.last_trade_candle = current.name
...
async def main():
    ...
    for config in symbol_configs:  # iterate the list
        ...
        bot = SymbolBot(symbol, merged_config, base_dir, client, data_folder_name="websocketDataRobustLong")
